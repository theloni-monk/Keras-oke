...
class Chars2Vec:
    ...
    def __init__(self, emb_dim, char_to_ix):
        ...
        lstm_input = tf.compat.v1.keras.layers.Input(shape=(None, self.vocab_size))

        x = tf.compat.v1.keras.layers.LSTM(emb_dim, return_sequences=True)(lstm_input)
        x = tf.compat.v1.keras.layers.LSTM(emb_dim)(x)

        self.embedding_model = tf.compat.v1.keras.models.Model(inputs=[lstm_input], outputs=x)

        model_input_1 = tf.compat.v1.keras.layers.Input(shape=(None, self.vocab_size))
        model_input_2 = tf.compat.v1.keras.layers.Input(shape=(None, self.vocab_size))

        embedding_1 = self.embedding_model(model_input_1)
        embedding_2 = self.embedding_model(model_input_2)
        x = tf.compat.v1.keras.layers.Subtract()([embedding_1, embedding_2])
        x = tf.compat.v1.keras.layers.Dot(1)([x, x])
        model_output = tf.compat.v1.keras.layers.Dense(1, activation='sigmoid')(x)

        self.model = tf.compat.v1.keras.models.Model(inputs=[model_input_1, model_input_2], outputs=model_output)
    ...
    def fit(self, word_pairs, targets,
        ...
        :param max_epochs: parameter 'epochs' of tf.compat.v1.keras model.
        :param patience: parameter 'patience' of callback in tf.compat.v1.keras model.
        :param validation_split: parameter 'validation_split' of tf.compat.v1.keras model.
        :param batch_size: parameter 'batch_size' of tf.compat.v1.keras model.
        '''
        ...
        x_1_pad_seq = tf.compat.v1.keras.preprocessing.sequence.pad_sequences(x_1)
        x_2_pad_seq = tf.compat.v1.keras.preprocessing.sequence.pad_sequences(x_2)

        self.model.fit([x_1_pad_seq, x_2_pad_seq], targets,
                       batch_size=batch_size, epochs=max_epochs,
                       validation_split=validation_split,
                       callbacks=[tf.compat.v1.keras.callbacks.EarlyStopping(monitor='val_loss', patience=patience)])
    ...
    def vectorize_words(self, words, maxlen_padseq=None):
        ...
        :param maxlen_padseq: parameter 'maxlen' for tf.compat.v1.keras pad_sequences transform.
        ...
        if len(new_words) > 0:
            ...
            embeddings_pad_seq = tf.compat.v1.keras.preprocessing.sequence.pad_sequences(list_of_embeddings, maxlen=maxlen_padseq)
